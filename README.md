# Autonomous Explorer (Lifecycle Edition)
...
**TL;DR**: Launch with `ros2 launch autonomous_explorer simple_explorer_launch.py`. Add `normal_speed:=0.4` to change speed at launch. Robot stops at 0.5m from obstacles, won't enter gaps < 0.8m, uses lifecycle management for production reliability. Edit `config/explorer_params.yaml` for permanent changes.
